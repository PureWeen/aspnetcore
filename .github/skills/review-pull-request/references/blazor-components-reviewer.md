...
This reference provides shared guidance for ASP.NET Core Blazor and Razor Components work under `src/Components/**` and `src/JSInterop/**` — rendering, lifecycle, render modes, JS interop, navigation, forms, and interactive Server circuits. For ordinary implementation or investigation, consult the relevant sections while following the requested task and applicable repository and area instructions. During an explicitly requested pull request review, the `review-pull-request` skill owns the read-only review orchestration and applies every required dimension.
...
- CHECK: For implementation work under `src/Components`, follow the Components sample and E2E workflow guidance; generic `src/JSInterop`-only work does not inherit that workflow. For review-only findings, use the source and contract evidence permitted by the review skill without requiring full-suite runs.
