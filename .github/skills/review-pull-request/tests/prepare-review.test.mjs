...
async function fixture(t)
{
    ...
    const dependencies = {
        api,
        fetch: (_repo, commits, store) => git(store, ['-c', 'fetch.unpackLimit=1000000', 'fetch', '--quiet', '--no-tags', '--no-auto-gc', repository, ...commits]),
    };
...
test('prepares distinct complete sides, inert target instructions, large files and dirty guidance', async t =>
{
    ...
    const workspace = path.join(f.options.output, 'workspace');
    const source = (role, name) => git(path.join(workspace, '.git'), ['show', '--no-ext-diff', '--no-textconv', `${manifest.target[role]}:${name}`]) + '\n';
    ...
    assert.equal((await source('head', 'src/Mode.cs')).trim(), 'Value.cs');
    ...
    assert.equal((await fs.stat(path.join(workspace, 'src/Mode.cs'))).isFile(), true);
    assert.equal(await fs.readFile(path.join(workspace, 'src/Value.cs'), 'utf8'), 'HEAD_VALUE\n');
    await assert.rejects(fs.stat(path.join(workspace, 'AGENTS.md')), { code: 'ENOENT' });
    await assert.rejects(fs.stat(path.join(workspace, '.github/copilot-instructions.md')), { code: 'ENOENT' });
    assert.equal((await fs.stat(path.join(workspace, '.github/skills/review-pull-request/scripts/prepare-review.mjs'))).isFile(), true);
    assert.equal(manifest.guidance.workingTreeChanges, true);
    assert.match(await fs.readFile(path.join(f.options.output, 'guidance/docs/CrossCuttingGuidance.md'), 'utf8'), /DIRTY_GUIDANCE/);
...
test('supports an immutable remote guidance selection without selecting the local checkout', async t =>
{
    ...
    const commit = f.commit({
        ...
        'AGENTS.md': 'REMOTE_RUNTIME_MUST_NOT_ACTIVATE\n',
        '.github/copilot/settings.json': '{"remoteRuntime":true}\n',
    });
    ...
    assert.match(await fs.readFile(path.join(options.output, 'guidance/docs/CrossCuttingGuidance.md'), 'utf8'), /REMOTE_GUIDANCE/);
    await assert.rejects(fs.stat(path.join(options.output, 'workspace/AGENTS.md')), { code: 'ENOENT' });
    assert.throws(() => git(path.join(options.output, 'workspace/.git'), ['cat-file', '-e', `${commit}^{commit}`]));
...
for (const [name, mutate] of [
    ...
    ['changed source', f => fs.appendFile(path.join(f.options.output, 'workspace/src/Value.cs'), 'MUTATED')],
    ['changed overlay', f => fs.appendFile(path.join(f.options.output, 'workspace/docs/CrossCuttingGuidance.md'), 'MUTATED')],
    ['unexpected instruction', async f =>
    {
        await fs.mkdir(path.join(f.options.output, 'workspace/src/Added'), { recursive: true });
        await fs.writeFile(path.join(f.options.output, 'workspace/src/Added/AGENTS.md'), 'UNEXPECTED_TARGET_INSTRUCTION\n');
    }],
    ['missing old original', async f =>
    {
        const blob = git(f.repository, ['rev-parse', `${f.mergeBase}:src/Deleted.cs`]);
        await fs.unlink(path.join(f.options.output, 'workspace/.git/objects', blob.slice(0, 2), blob.slice(2)));
    }],
    ...
    ['partial manifest', async f =>
    {
        ...
        delete manifest.trees.mergeBase;
    ...
    ['wrong source role', async f =>
    {
        const filename = path.join(f.options.output, 'manifest.json');
        const manifest = JSON.parse(await fs.readFile(filename));
        manifest.trees.head = manifest.trees.baseTip;
        await fs.writeFile(filename, JSON.stringify(manifest));
    }],
])
{
...
test('rejects directory, case and Windows aliases without needing an exported suffix', () =>
{
    for (const names of [
        ['x', 'x/child'], ['x/child', 'x'],
    ...
    assert.doesNotThrow(() => checkPaths(['x', 'x.source/child']));
});
